...
/**
 * This class starts a server to listen and accept connections from clients over a socket connection. 
 */
public class Server {
     ...
     * 	binds a server to a port on the local address and initialilzes lists 
     * @param
     * 	int - port -- port to bind to (0 <= int <= 65535)
     * @throws
     * 	IOException - if socket cannot be bound to port 
     */
     ...
     * 	listens for a connection then creates a new thread to construct a handler for that connection as well as updates all the lists
     */
    public void serve() {
        ...
        while (true)
            try {
                ...
                //create a new thread to create a connection (so the server is free to accept another connection)
                new Thread() {
                    public void run() {
                        try {
                            ...
                            //attempt to add the connection (client) to the list of connected clients
                        ...
                        //If there is an error (like the user already exists, or user disconnects at startup sequence)
                        } catch (Exception e) {
                            try {
            	...
            	//really... this shouldnt ever be ran... really... unless something FUBAR happened.
